/**
 * @Author: enoyao
 * @Date: 2021-07-29 14:45:32
 */

import React from 'react';
import './index.css';

const PreviewHolder = (): JSX.Element => {
    return (
        <div className="preview-holder">
            <div className="preview-holder-container">
                <div className="preview-holder-universe">
                    <span className="universe-icon" style={{ fontSize: '28px' }}>
                        <svg
                            width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M4 2.5C4 1.94772 4.44772 1.5 5 1.5H14.8608C14.9949 1.5 15.1234 1.55387 15.2174 1.64951L19.8566 6.36941C19.9485 6.46292 20 6.58879 20 6.7199V21.5C20 22.0523 19.5523 22.5 19 22.5H5C4.44772 22.5 4 22.0523 4 21.5V2.5Z"
                                fill="#3370FF"></path>
                            <path
                                d="M15 1.51978C15.0817 1.54345 15.1567 1.58778 15.2174 1.64952L19.8566 6.36942C19.8946 6.40806 19.9256 6.45223 19.949 6.50001H16.1351C15.5082 6.50001 15 5.99179 15 5.36488V1.51978Z"
                                fill="#245BDB"></path>
                            <path
                                d="M8.07282 10.1818H15.6546C15.745 10.1818 15.8183 10.2551 15.8183 10.3455V11.1091C15.8183 11.1995 15.745 11.2727 15.6546 11.2727H8.07282C7.98244 11.2727 7.90918 11.1995 7.90918 11.1091V10.3455C7.90918 10.2551 7.98244 10.1818 8.07282 10.1818ZM8.07282 13.4546H15.6546C15.745 13.4546 15.8183 13.5278 15.8183 13.6182V14.3818C15.8183 14.4722 15.745 14.5455 15.6546 14.5455H8.07282C7.98244 14.5455 7.90918 14.4722 7.90918 14.3818V13.6182C7.90918 13.5278 7.98244 13.4546 8.07282 13.4546ZM8.07282 16.7273H12.1092C12.1996 16.7273 12.2728 16.8005 12.2728 16.8909V17.6546C12.2728 17.7449 12.1996 17.8182 12.1092 17.8182H8.07282C7.98244 17.8182 7.90918 17.7449 7.90918 17.6546V16.8909C7.90918 16.8005 7.98244 16.7273 8.07282 16.7273Z"
                                fill="white"></path>
                        </svg>
                    </span>
                </div>
                <div className="ellipsis">未命名文档</div>
            </div>
        </div>
    );
};
export default PreviewHolder;
